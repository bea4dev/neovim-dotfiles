...
local function execute_project(pat, rep)
  ...
  local choice = vim.fn.confirm(
    ("Replace in %d file(s) under %s?"):format(#files, cwd),
    "&Yes\n&No", 2)
  ...
  local ok, err = pcall(vim.cmd,
    ("silent! argdo keeppatterns %%s/\\v%s/%s/ge | update"):format(pat_e, rep_e))

  pcall(vim.cmd, "silent! %argdelete")
  if #saved > 0 then
    local q = {}
    for _, f in ipairs(saved) do table.insert(q, vim.fn.fnameescape(f)) end
...
function M.open(opts)
  ...
  local title = mode == "project"
    and " Replace · ENTIRE PROJECT (regex) "
    or  " Replace · File (regex) "
  ...
  vim.keymap.set("i", "<BS>", function()
    ...
    if ccol <= prefix_len then return "" end
    return "<BS>"
