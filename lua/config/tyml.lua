...
local function start_tyml(bufnr, force)
  ...
  if not force then
    for _, c in ipairs(vim.lsp.get_clients({ bufnr = bufnr })) do
      if c.name ~= "tyml" then return end
    end
  end
  ...
  if not bin then
    if not notified_missing then
      ...
      vim.notify(
        "[tyml-lsp] tyml-lsp-server が見つかりません",
  ...
  }, { bufnr = bufnr, reuse_client = function(client) return client.name == "tyml" end })
end
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function(ev)
    ...
    if force then
      ...
      vim.schedule(function() start_tyml(ev.buf, false) end)
    end
  end,
})
